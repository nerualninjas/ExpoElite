...
const ChatBox = () => {
    ...
    const { messageLogs, messegeLoading, refetch, messegePending } = useGetMessage({ senderEmail: user.email });
    ...
    const handleSendMessage = async () => {
        if (message.trim() !== '') {
            console.log(message);
            const res = await axiosPublic.post(`/sendMessage?senderEmail=${user.email}&user=${user.email}`, {
                message: message
            });
